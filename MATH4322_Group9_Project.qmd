...
Now, we proceed to train the multiple linear model and evaluate the prediction accuracy. We will perform a randomized 
...
In this section, we focused on utilizing a random forest model to see if it was a better fit in predicting the interstate highway traffic volume. 
```{r}
